                            ...
                            <div class="row mt-4">
                                <div class="col-lg-8">
                                    ...
                                    <p class="section-lead">{{ ucwords($order->payment_method) }}<br> @if(
                                        $order->transaction->transaction_id )({{ 'Transaction ID: ' .
                                            $order->transaction->transaction_id }})@endif</p>
...
@push( 'scripts' )
    <script>
        ...
        const orderStateChange = (target) => {
            ($ => {
                $(() => {
                    ...
                    let id_target = "#order_status";

                    if (target === 'payment_status') {
                        url = "{{ route('admin.order.change-payment-status') }}";
                        id_target = "#payment_status";
                    }

                    $("body").on("change", id_target, e => {
        ...
        orderStateChange('payment_status');
        printInvoice();
    </script>
@endpush
